...
class _Meshes():
    ...
    def _init(ctx: moderngl.Context):
        ...
        _Meshes._texture = skittle.render.mesh.TextureMesh(ctx, None, 500, 500)
...
def texture(camera: skittle.camera.Camera, surface: pygame.Surface, pos: glm.vec2, scale: glm.vec2 = glm.vec2(1, 1), color: skittle.color.Color = skittle.color.WHITE, rotation: float = 0.0, layer: int = 0, overlay: bool = False):
    def _sub():
        _Meshes._texture.load_texture(surface)
        _Meshes._texture._render_now(camera, pos, scale, color, rotation, overlay)
    camera.submit(_sub, camera.calc_layer(layer, overlay))