...
"""
    ...
    def render(self, camera: skittle.render.Camera, overlay: bool = False, mode: int = moderngl.TRIANGLES, no_cam: bool = False):
        ...
        if no_cam:
            ...
            camera.await_completion(self._render_now, overlay, mode=mode, layer=layer)
...
"""
    ...
    def __init__(self, ctx: moderngl.Context, vertex: str = OldMesh.VERTEX, fragment: str = QUADMESH_FRAGMENT) -> None:
        vertices, indices = OldMesh.uv_quad(1, 1)
...
"""
    ...
    def __init__(self, ctx: moderngl.Context, w: int, h: int, u0: float, v0: float, u1: float, v1: float, texture: pygame.Surface, vertex: str = OldMesh.VERTEX, fragment: str = TEXTURE_FRAGMENT) -> None:
        vertices, indices = OldMesh.uv_quad(w, h, u0, v0, u1, v1)
...
class SpriteQuad(TextureMesh):
    def __init__(self, ctx: moderngl.Context, texture: pygame.Surface, vertex: str = OldMesh.VERTEX, fragment: str = TextureMesh.TEXTURE_FRAGMENT) -> None:
        super().__init__(ctx, texture.width, texture.height, 0, 0, 1, 1, texture, vertex, fragment)

class SpritesheetQuad(TextureMesh):
    def __init__(self, ctx: moderngl.Context, spritesheet: skittle.resource.Spritesheet, frame: tuple[int, int] = (0, 0), vertex: str = OldMesh.VERTEX, fragment: str = TextureMesh.TEXTURE_FRAGMENT) -> None:
        self.frame = frame
    ...
    def set_sprite(self, frame: tuple[int, int]):
        ...
        vertices, _ = OldMesh.uv_quad(self.spritesheet.sprite_w, self.spritesheet.sprite_h, u0, v0, u1, v1)
